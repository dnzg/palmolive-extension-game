...
ga("create", "UA-177535495-2", "auto", {
  cookieFlags: "SameSite=None; Secure",
});
ga("set", "anonymizeIp", true, { cookieFlags: "SameSite=None; Secure" });
...
helper.onAuthorized(async function (auth) {
  ...
  ga("send", "event", "channel", "channelname", auth.channelId, {
    cookieFlags: "SameSite=None; Secure",
  });

  const username = await axios
    .get("https://api.twitch.tv/kraken/users/" + twitchUsername, {
      headers: {
...
async function dbWrite(userid, score) {
  if (isAuthed) {
    ...
    if (userid !== null && userid !== undefined && userid !== "") {
      ...
      await db
        .ref("users/" + userid + "/gamesCount")
        .transaction(function (gamesCount) {
          return gamesCount + 1;
        });
      await db
        .ref("users/" + userid + "/scoreTotal")
...
$("#leaderboardButton").on("click", function () {
  leaderboard(true);
  ga("send", "event", "leaderboard", "visit", "from-topMenu", {
    cookieFlags: "SameSite=None; Secure",
  });
});
...
$("#go2").on("click", function () {
  leaderboard(true);
  ga("send", "event", "leaderboard", "visit", "from-gameOver", {
    cookieFlags: "SameSite=None; Secure",
  });
});
