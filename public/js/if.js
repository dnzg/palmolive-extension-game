...
window.Twitch.ext.onAuthorized((auth) => {
    ...
    req.onreadystatechange = function () {
        if (req.readyState === 4 && req.status === 200) {
            ...
            db.ref("users/"+userTwitch).on("value", (snapshot) => {
    ...
    let uid = auth.userId.slice(1);
    req.open("GET", "https://api.twitch.tv/kraken/users/"+uid, true);
    req.setRequestHeader("Accept", "application/vnd.twitchtv.v5+json");
    req.setRequestHeader("Client-ID", auth.clientId);
    req.send(null);
});

function ratingWrite(num) {
    if(num > recordScore) dbWrite(userTwitch, num);
}
